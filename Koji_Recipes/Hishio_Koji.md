...
~~~python
x=100; I='醤 米麹 水 塩 合計'.split(); r=[1,1,1]; s_s=[0.16,0,0]; salt=0.13; import clipboard; b_r=r[0]; r_norm=[v/b_r for v in r]; s_r=sum(r_norm); liq_s=sum(v*s for v,s in zip(r_norm, s_s)); t_r=max(s_r, (s_r-liq_s)/(1-salt)); s_amt=max(0, t_r-s_r); act_s=(liq_s+s_amt)/t_r; R=r_norm+[s_amt, t_r]; N=[f'塩分:{round(s*100,1)}%' if s != 0 else '' for s in s_s]+[f'塩分:100%']+[f'全体塩分:{round(act_s*100,1)}%']; res="|材料|割合|分量|%|備考|\n|:-:|:-:|:-:|:-:|:-:|\n"+"\n".join(f"|**{n}**|{round(v*b_r,2) if i<len(r) else round(v,2)}|{round(x*v,1)}{'ml' if n in['水','醤油','酒'] else 'g'}|{round(v/t_r*100,1)}%|{note}|" for i,(n,v,note) in enumerate(zip(I,R,N))); clipboard.set(res)
~~~
...
|    材料     | 割合  |   分量   |   %    |    備考     |
| :-------: | :-: | :----: | :----: | :-------: |
|   **醤**   | 1.0 | 100.0g | 27.8%  | 塩分:16.0%  |
| **発酵トマト** | 0.5 | 50.0g  | 13.9%  |  塩分:2.0%  |
|  **林檎酢**  | 1.0 | 100.0g | 27.8%  |           |
|  **甘酒**   | 1.0 | 100.0g | 27.8%  |           |
|   **塩**   | 0.1 | 10.0g  |  2.8%  |  塩分:100%  |
|  **合計**   | 3.6 | 360.0g | 100.0% | 全体塩分:7.5% |
|           |     |        |        |           |
|    鷹の爪    |     |        |        |           |
| ブラックペッパー  |     |        |        |           |
|    陳皮     |     |        |        |           |
