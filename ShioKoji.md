...
~~~python
x=185; I='生姜 白米麹 水 塩 合計'.split(); r=[1,2,1.7]; salt=0.13; import clipboard; s_r=sum(r); t_r=s_r/(1-salt); R=r+[t_r-s_r,t_r]; res="|材料|割合|分量|%|\n|:-:|:-:|:-:|:-:|\n"+"\n".join(f"|**{n}**|{round(v,2)}|{round(x*v,1)}{'ml' if n=='水' else 'g'}|{round(v/t_r*100,1)}%|" for n,v in zip(I,R)); clipboard.set(res)
~~~
...
|**水**|2.5|462.5ml|39.5%|
|**塩**|0.8|152.0g|13.0%|
|**合計**|6.3|1169.5g|100.0%|

##### PyKeysのREPL用ワンライナー
実行するとクリップボードにテーブルがコピーされる。

~~~python
x=185; I='生姜 白米麹 水 塩 合計'.split(); r=[1,2,2.5]; salt=0.13; import clipboard; s_r=sum(r); t_r=s_r/(1-salt); R=r+[t_r-s_r,t_r]; res="|材料|割合|分量|%|\n|:-:|:-:|:-:|:-:|\n"+"\n".join(f"|**{n}**|{round(v,2)}|{round(x*v,1)}{'ml' if n=='水' else 'g'}|{round(v/t_r*100,1)}%|" for n,v in zip(I,R)); clipboard.set(res)
~~~
...
~~~python
x=120; I='にら 白米麹 水 塩 合計'.split(); r=[1,2,3]; salt=0.13; import clipboard; s_r=sum(r); t_r=s_r/(1-salt); R=r+[t_r-s_r,t_r]; res="|材料|割合|分量|%|\n|:-:|:-:|:-:|:-:|\n"+"\n".join(f"|**{n}**|{round(v,2)}|{round(x*v,1)}{'ml' if n=='水' else 'g'}|{round(v/t_r*100,1)}%|" for n,v in zip(I,R)); clipboard.set(res)
~~~
...
x=200; I='麹 醤油 塩 合計'.split(); r=[1,1]; salt=0.08; s_s=0.16; import clipboard; s_r=sum(r); t_r=(s_r-r[1]*s_s)/(1-salt); R=r+[t_r-s_r,t_r]; res="|材料|割合|分量|%|\n|:-:|:-:|:-:|:-:|\n"+"\n".join(f"|**{n}**|{round(v,2)}|{round(x*v,1)}{'ml' if n=='醤油' else 'g'}|{round(v/t_r*100,1)}%|" for n,v in zip(I,R)); clipboard.set(res)
~~~
